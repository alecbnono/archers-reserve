...
import { getAllReservations } from "../reservation/reservation.service.js";
...
export async function searchUsers(query) {
    const pattern = `%${query}%`;
    const result = await pool.query(`SELECT user_id, username, first_name, last_name, email, role
     FROM "user"
 ...
 *
 */
export function unmergeTimeslotRanges(timeSlotString) {
    if (!timeSlotString || timeSlotString === "")
        return [];
